...
| **6. Agent (ReAct + OpenClaw)** | Orchestration & conversational UI | ReAct loop (Thought→Action→Observation→Answer). 9 intent classifiers. Redis-backed conversation memory (20-turn window). OpenClaw integration for MCP tools and WDK skill. SSE streaming for chat. |
| **7. MCP Server** | Tool exposure for agents | 9 tools: `get_portfolio`, `run_optimization`, `get_optimization_plan`, `broadcast_signed_tx`, `quant_var`, `quant_moments`, `explain_formula`, `explain_concept`, `explain_strategy`. HTTP-based for OpenClaw consumption. |
...
| OpenClaw UI | http://localhost:18789 | Agent control panel (MCP tools + WDK skill) |
| MCP Server | http://localhost:3001 | Tool server for OpenClaw |
...
- **Signal generation engine** with 3 strategies (momentum, mean-reversion, breakout)
...
- **ReAct agent loop** — Thought→Action→Observation→Answer (max 5 iterations)
- **9 intent classifiers** — routes queries to appropriate tools
...
| **[docs/OPENCLAW_INTEGRATION.md](docs/OPENCLAW_INTEGRATION.md)** | OpenClaw orchestration, MCP tools, agent configuration |
...
│   └── app/                   # ReAct agent, GraphRAG, scrapers (in ai-core)
│
├── gateway-wdk/
│   └── src/
...
│
├── mcp-server-yield-agent/    # MCP tools for OpenClaw
├── ai-core/
